...
real*8 :: xi,mu_eg,lam,omega_c,beta,part,mu_dd,mu_aa,R_0,diab,fluc,ws
...
fluc = (xi **2) * (2*omega_c) * (1.0/(beta * omega_c**2))
ws  =  5.26035E-3
diab = sqrt(diab**2 + fluc) 
...
A(:,:) = 0.0
 ...
 do n = 1,nfock
    do m = 1,nfock
       ...
       vda(n,m) = (diab)*(sij(n,m))  
     
       ! prefactor term corresponding to each channel                                                                                    

       A(n,m) = 2.0*pi*((vda(n,m))**2)*(1.0/sqrt(4.0*pi*lam*(1.0/beta)))  

       A(n,m) = (A(n,m)*41341.37)  !/ (1.0 + 4.0 * (3.1415 / (lam *ws) )* diab**2)
...
!      write(*,*) "n=",real(n),  exp(-beta*real(n)*omega_c)
   ...
   do g = 1,nstep
      ...
      open(unit=110, file="beratan_wc0.2ev_g0.01_diab0.005.out")
